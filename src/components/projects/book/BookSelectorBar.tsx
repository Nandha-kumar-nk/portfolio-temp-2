...
interface BookSelectorBarProps {
...
export const BookSelectorBar: React.FC<BookSelectorBarProps> = ({
  projects,
  currentIndex,
  onSelectProject,
  onNext,
  ...
  return (
    <div className="w-full max-w-4xl mx-auto my-6 px-4 flex flex-col sm:flex-row items-center justify-between gap-4 select-none">
      ...
      <button
        onClick={onPrev}
        className="group flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-900/90 border border-slate-800 text-slate-300 text-xs font-mono font-bold tracking-wider hover:text-cyan-300 hover:border-cyan-500/50 hover:bg-cyan-950/40 transition-all duration-300 active:scale-95 shadow-lg"
      >
        ...
        {projects.map((proj, idx) => {
          ...
          return (
            <button
              ...
              className={`relative flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl border font-mono text-xs transition-all duration-300 whitespace-nowrap active:scale-95 ${
                isActive
