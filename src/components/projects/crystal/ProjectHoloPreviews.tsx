...
interface ProjectHoloPreviewsProps {
...
export const ProjectHoloPreviews: React.FC<ProjectHoloPreviewsProps> = ({
  projects,
  selectedIndex,
  onSelectProject,
  disabled = false,
  ...
  const desktopPositions = [
    {
      // 1. Top-Left
      className: 'hidden xl:flex absolute left-4 2xl:left-10 top-4 [transform:perspective(900px)_rotateY(14deg)_rotateX(6deg)]',
      hoverTransform: 'hover:[transform:perspective(900px)_rotateY(0deg)_rotateX(0deg)_scale(1.04)]',
    },
    {
      // 2. Top-Right
      className: 'hidden xl:flex absolute right-4 2xl:right-10 top-4 [transform:perspective(900px)_rotateY(-14deg)_rotateX(6deg)]',
      hoverTransform: 'hover:[transform:perspective(900px)_rotateY(0deg)_rotateX(0deg)_scale(1.04)]',
    },
    {
      // 3. Bottom-Left
      className: 'hidden xl:flex absolute left-4 2xl:left-10 bottom-4 [transform:perspective(900px)_rotateY(14deg)_rotateX(-6deg)]',
      hoverTransform: 'hover:[transform:perspective(900px)_rotateY(0deg)_rotateX(0deg)_scale(1.04)]',
    },
    {
      // 4. Bottom-Right
      className: 'hidden xl:flex absolute right-4 2xl:right-10 bottom-4 [transform:perspective(900px)_rotateY(-14deg)_rotateX(-6deg)]',
      hoverTransform: 'hover:[transform:perspective(900px)_rotateY(0deg)_rotateX(0deg)_scale(1.04)]',
    },
  ];
  ...
  return (
    <>
        ...
        return (
          <div
            ...
            className={`${config.className} z-20 transition-all duration-500 ease-out ${
              isMoving ? 'scale-95 opacity-35 translate-y-1' : 'opacity-70 hover:opacity-100'
            }`}
          >
            <FloatingDisplayCard
              project={item.proj}
      ...
      {/* ========================================================================= */}
      {/* TABLET (768px-1199px): ONLY 2 SURROUNDING PREVIEWS (Left and Right)       */}
      {/* ========================================================================= */}
      <div className="hidden md:flex xl:hidden absolute left-2 top-1/2 -translate-y-1/2 z-20 opacity-65 hover:opacity-100 transition-all duration-300">
      ...
      <div className="hidden md:flex xl:hidden absolute right-2 top-1/2 -translate-y-1/2 z-20 opacity-65 hover:opacity-100 transition-all duration-300">
...
interface FloatingDisplayCardProps {
  project: ProjectItem;
...
const FloatingDisplayCard: React.FC<FloatingDisplayCardProps> = ({
  project,
  originalIndex,
  onClick,
  disabled,
  ...
  return (
    <button
      ...
      className={`group relative ${
        compact ? 'w-32 p-1.5' : 'w-36 2xl:w-42 p-2'
    ...
    >
      <div className="relative w-full h-13 2xl:h-15 rounded-lg overflow-hidden border border-slate-800 group-hover:border-cyan-400/50 transition-colors bg-slate-900">
        <img
          src={project.image}
          alt={project.title}
          loading="lazy"
      ...
      <div className="mt-1 flex flex-col">
        <span className="text-[10.5px] 2xl:text-[11.5px] font-bold text-white uppercase tracking-wider truncate group-hover:text-cyan-300 transition-colors">
          {project.shortName || project.title}
        </span>
        <span className="text-[8.5px] 2xl:text-[9.5px] font-mono text-cyan-400/90 truncate">
          {project.category}
        </span>
      </div>
    </button>
